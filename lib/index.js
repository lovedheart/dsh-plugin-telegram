...
import { chunkText, markdownToTelegramHtml, guardConvertedLength, sessionShortIds, wrapDisplay } from './text.js';
import { ProgressIndicator, summarizeToolArgs, tailOf, compactText } from './progress.js';
import { SubagentBoard, DEFAULT_REFRESH_MS } from './subagents.js';
import { randomUUID } from 'node:crypto';

// Re-export the indicator + its text helpers so existing importers (tests +
// the module's public surface) are unchanged after the extraction into
// progress.js.
export { ProgressIndicator, summarizeToolArgs, tailOf, compactText };
import { createInboundMediaModule, sniffImageMediaType } from './inbound-media.js';
// Re-export the image sniffer (tests import it from the module root).
export { sniffImageMediaType };
...
// message. Defined at module scope so every reply path (command handlers AND
// notifyTurnFailure) can use it — it used to live inside one nested helper,
// which made `escapeHtml is not defined` crash the turn-failure notice and
// swallow the very error it was reporting.
function escapeHtml(s) {
  return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

// dsh session format v4 (host ≥ 0.1.7) REJECTS the retired generic
// `source.kind: 'plugin'` wrapper ("format v4 message requires a
// producer-owned source kind") — every injected turn then dies instantly.
// The v3→v4 migration maps a plugin wrapper to `plugin:<name>`; new sends
// must declare that producer-owned kind directly.
const messageSourceKind = `plugin:${name}`;
...
export async function apply(ctx, config) {
  ...
  //       chatId (composite-key reverse lookup; bit-compatible with the
  //       legacy single-bot config);
  //   (3) v0.6.5 — when MULTIPLE bots route the SAME chatId (two bots sharing
  //       one private chat), the chat lookup is ambiguous and used to always
  //       pick the first bot, so bot B's agent's sends left through bot A.
  //       The CALLING agent (exec.agent, set by the agent loop) now decides:
  //       resolve its owning bot via botIdForAgent and send on THAT bot.
  //   (4) the legacy first-bot client (bit-compatible single-bot fallback).
  // Returns { botId, client, error } — exactly one of client/error is non-null.
  // NOTE: `client` / `clientFor` / `botIdForChat` / `botIdForAgent` are all in
  // the same apply() scope (declared below); resolveBotClient only runs at
  // tool-execute time, after they are initialized, so the closures are safe.
  function resolveBotClient(botArg, chatId, callerAgent) {
    ...
    // v0.6.5: chatId → owning bot(s), collected across the (bot, chat)
    // routes. A chatId routed for exactly ONE bot has unambiguous ownership
    // and that bot sends (bit-compatible with the pre-v0.6.5 behavior for
    // distinct chats). When TWO bots route the SAME chatId (two bots sharing
    // one private chat — both owners connected), the reverse lookup is
    // Map-order-dependent and always picks the FIRST bot — sending bot B's
    // agent's photos through bot A, the reported cross-bot bug. In that
    // ambiguous case the CALLING agent decides instead: media must go back
    // out the bot the user actually talked to.
    const cid = String(chatId ?? '');
    const chatOwners = [];
    for (const key of chatAgents.keys()) {
      const sep = key.indexOf('::');
      const kc = sep >= 0 ? key.slice(sep + 2) : key;
      if (kc !== cid) continue;
      const ownerBid = sep >= 0 ? key.slice(0, sep) : 'default';
      const oe = botRegistry.get(ownerBid);
      if (oe && oe.client) chatOwners.push(ownerBid);
    }
    if (chatOwners.length === 1) {
      return { botId: chatOwners[0], client: botRegistry.get(chatOwners[0]).client, error: null };
    }
    // Ambiguous (multiple bots own this chatId) or no chat route at all:
    // resolve the CALLING agent's bot (subagents resolve through their
    // parentSession chain).
    const agentBotId = botIdForAgent(callerAgent);
    if (agentBotId) {
      const e = botRegistry.get(agentBotId);
      if (e && e.client) return { botId: agentBotId, client: e.client, error: null };
    }
    // Shared chat but the calling agent is unknown (e.g. a tool call from a
    // web agent with no (bot, chat) route): the first connected owner —
    // the same choice the old chat-only lookup made.
    if (chatOwners.length >= 1) {
      return { botId: chatOwners[0], client: botRegistry.get(chatOwners[0]).client, error: null };
    }
  ...
  function log(level, ...args) {
    ...
    // Mirror to stderr so plugin logs are journal-visible (same gating rule
    // as below — never fires for non-error when verbose is off).
    console[level === 'debug' || level === 'info' ? 'log' : level](`[${name}/diag]`, ...args);
  ...
  async function watchDirectReply(agent, botId, chatId, telegramMessageId) {
    try {
      ...
      // C 兜底：检测纯文本选项 → 改发 inline keyboard 卡片
      const detection = detectTextOptions(prefixed);
      if (detection) {
        log('info', `C-fallback: detected text options in direct reply for chat ${chatId} (${detection.options.length} opts, ${detection.multiSelect ? 'multi' : 'single'})`);
        await sendSyntheticQuestionCard(botId, chatId, detection, { replyToMessageId: telegramMessageId });
        return;
      }
  ...
  async function notifyTurnFailure(agent, botId, chatId, baseline, opts = {}) {
    try {
      ...
      // Log FIRST (before any formatting can throw) so the root error always
      // reaches the journal even if the notice path fails.
  ...
  // -----------------------------------------------------------------------
  // C 兜底：检测 agent 最终文本里的 "A. … B. … C. …" 选项列表，
  // 若本回合未调 ask_user_question，则把纯文本选项改写为 inline keyboard 卡片。
  // 用户点按钮后，选择结果作为新消息注入 agent（开新回合）。
  // -----------------------------------------------------------------------
  const SYNTH_Q_PREFIX = 'tsq1:'; // synthetic question callback prefix
  const syntheticQuestions = new Map(); // chatId -> { text, options, messageId }

  /**
   * Detect "A. …\nB. …\nC. …" option lists in agent text.
   * Returns { question, options: [{key, label}], multiSelect } or null.
   * Gates (to suppress false positives):
   *   - ≥ 2 options, each matching /^[A-Z][.、)）]\s*\S/
   *   - Options must be at/near the END of the text (last 60% of lines)
   *   - Text must contain a question cue (？/问号/选/哪/以下/正确/错误/真假)
   */
  function detectTextOptions(text) {
    if (!text || typeof text !== 'string') return null;
    const lines = text.split('\n');
    // Find option lines: A. / B. / C. / A、/ A) etc.
    const optRe = /^([A-Z])[.、)）]\s*(.+)$/;
    const options = [];
    let firstOptIdx = -1;
    for (let i = 0; i < lines.length; i++) {
      const m = lines[i].trim().match(optRe);
      if (m) {
    ...
    // Options must occupy the latter part of the message (last 60%+ of lines)
    const optSpan = lines.length - firstOptIdx;
    if (optSpan > lines.length * 0.7) return null; // options are the whole message → probably a list, not a question
    // Question cue: must be a question (not just a list)
    const head = lines.slice(0, firstOptIdx).join(' ');
    if (!/[\?？]|选|哪|以下|正确|错误|真假|是不是|判断|脑暴|题目|quiz|which|select|choose|multiple/i.test(head)) return null;
    // Detect multi-select cue
    const multiSelect = /多选|可多选|哪些|任选|多个|multiple|select all/i.test(head);
    // Question text = everything before the first option
    const question = head.trim() || text.slice(0, firstOptIdx).trim();
    return { question, options, multiSelect };
  }

  /**
   * Send a synthetic question card (inline keyboard) for a detected text-option
   * reply. Returns the sent messageId or null on failure.
   */
  async function sendSyntheticQuestionCard(botId, chatId, detection, opts = {}) {
    const { question, options, multiSelect } = detection;
    const tgClient = clientFor(botId);
    ...
    // Build text
    const parts = ['❓ 需要你选择'];
    if (question) parts.push('', esc(question));
    // Build keyboard rows (one button per option, auto-wrap by display width)
    const keyboard = [];
    // Simple: 1 option per row for CJK-friendly rendering
    const optRow = options.map((o) => ({
      text: `${o.key}. ${o.label}`,
      callback_data: `${SYNTH_Q_PREFIX}${key}:${o.key}`,
    }));
    // Wrap: max 1 per row (CJK labels can be long)
    for (const b of optRow) keyboard.push([b]);
    // Final row: submit + cancel (or just submit for multi)
    const finalRow = [];
    if (multiSelect) {
      finalRow.push({ text: '✅ 提交选择', callback_data: `${SYNTH_Q_PREFIX}${key}:submit` });
    } else {
      // Single-select: tapping an option IS the submit (no separate submit button)
    }
    finalRow.push({ text: '❌ 取消', callback_data: `${SYNTH_Q_PREFIX}${key}:cancel` });
    keyboard.push(finalRow);

    const text = parts.join('\n');
    try {
      const res = await tgClient.sendMessage({
        chatId: chatIdStr,
        text,
        parseMode: 'HTML',
        replyToMessageId: opts.replyToMessageId,
      ...
      syntheticQuestions.set(chatIdStr, { key, question, options, multiSelect, messageId, selected: new Set() });
      log('info', `Synthetic question card sent for chat ${chatIdStr} (${options.length} options, ${multiSelect ? 'multi' : 'single'})`);
      return messageId;
    } catch (err) {
      log('warn', `Synthetic question card send failed for chat ${chatIdStr}: ${err.message}`);
      return null;
    }
  }

  /**
   * Rebuild the keyboard for a synthetic card (option buttons + submit/cancel).
   */
  function buildSynthKeyboard(entry) {
    const { key, options, multiSelect } = entry;
    const kb = options.map((o) => [
      { text: `${o.key}. ${o.label}`, callback_data: `${SYNTH_Q_PREFIX}${key}:${o.key}` },
    ]);
    const finalRow = [];
    if (multiSelect) {
      finalRow.push({ text: '✅ 提交', callback_data: `${SYNTH_Q_PREFIX}${key}:submit` });
    }
    finalRow.push({ text: '❌ 取消', callback_data: `${SYNTH_Q_PREFIX}${key}:cancel` });
    kb.push(finalRow);
  ...
  /**
   * Handle callback queries from synthetic question cards.
   * Returns true when handled (caller should ack), false to fall through.
   */
  async function handleSyntheticQuestionCallback(botId, query) {
    const data = query?.data;
    if (!data || !data.startsWith(SYNTH_Q_PREFIX)) return false;
    const parts = data.slice(SYNTH_Q_PREFIX.length).split(':');
    const key = parts[0];
    const action = parts.slice(1).join(':');
    const chatId = query?.message?.chat?.id;
    const chatIdStr = String(chatId);
    const entry = syntheticQuestions.get(chatIdStr);
    ...
    try {
      // ── Cancel ──
      if (action === 'cancel') {
        await tgClient.editMessageText(chatIdStr, entry.messageId,
          `${entry.question ? escapeHtml(entry.question) + '\n\n' : ''}❌ 已取消`,
          'HTML', { inline_keyboard: [] });
        syntheticQuestions.delete(chatIdStr);
        await tgClient.answerCallbackQuery(query.id, '已取消');
        return true;
      }
      ...
      if (action === 'submit') {
        if (!entry.multiSelect) {
          await tgClient.answerCallbackQuery(query.id, '这是单选题，直接点选项即可');
          return true;
        }
        if (!entry.selected.size) {
          await tgClient.answerCallbackQuery(query.id, '请先选择至少一个选项');
          return true;
        }
        // Finalize: show selection, remove keyboard, inject
        const labels = [...entry.selected].map((k) => entry.options.find((o) => o.key === k)?.label || k);
        const answerText = `[用户选择] 答案：${[...entry.selected].join(' + ')}（${labels.join('、')}）`;
        await tgClient.editMessageText(chatIdStr, entry.messageId,
          `${entry.question ? escapeHtml(entry.question) + '\n\n' : ''}✅ 已选：${escapeHtml(labels.join('、'))}`,
          'HTML', { inline_keyboard: [] });
        syntheticQuestions.delete(chatIdStr);
        const agent = resolveChatAgent(botId, chatIdStr);
        if (agent && (typeof agent.followup === 'function' || typeof agent.send === 'function')) {
          const userMessage = {
            role: 'user',
            content: [{ type: 'text', text: answerText }],
            source: { kind: messageSourceKind },
            id: `telegram-synth-${Date.now()}-${key}`,
          };
          if (typeof agent.followup === 'function') agent.followup(userMessage);
          else agent.send(userMessage, 'next-turn', true);
          log('info', `Synthetic Q answer injected (multi) for chat ${chatIdStr}: ${answerText}`);
        }
        await tgClient.answerCallbackQuery(query.id, '已提交');
        return true;
      }

      // ── Option tap ──
      const opt = entry.options.find((o) => o.key === action);
      if (!opt) {
        await tgClient.answerCallbackQuery(query.id, '未知选项');
        return true;
      }

      if (!entry.multiSelect) {
        // Single-select: tap IS submit
        const answerText = `[用户选择] 答案：${action}（${opt.label}）`;
        await tgClient.editMessageText(chatIdStr, entry.messageId,
          `${entry.question ? escapeHtml(entry.question) + '\n\n' : ''}✅ 已选：${escapeHtml(opt.label)}`,
          'HTML', { inline_keyboard: [] });
        syntheticQuestions.delete(chatIdStr);
        const agent = resolveChatAgent(botId, chatIdStr);
        if (agent && (typeof agent.followup === 'function' || typeof agent.send === 'function')) {
          ...
          log('info', `Synthetic Q answer injected (single) for chat ${chatIdStr}: ${answerText}`);
        }
        await tgClient.answerCallbackQuery(query.id, `已选 ${opt.label}`);
        return true;
      }

      // Multi-select: toggle the option, update card text, keep keyboard
      if (entry.selected.has(action)) {
        entry.selected.delete(action);
      } else {
        entry.selected.add(action);
      }
      const chosenLabels = [...entry.selected].map((k) => entry.options.find((o) => o.key === k)?.label || k);
      const hint = chosenLabels.length
        ? `🔘 已选：${escapeHtml(chosenLabels.join('、'))}（可继续点选，完成后点「✅ 提交」）`
        : `🔘 可多选，选完点「✅ 提交」`;
      await tgClient.editMessageText(chatIdStr, entry.messageId,
        `${entry.question ? escapeHtml(entry.question) + '\n\n' : ''}${hint}`,
        'HTML', buildSynthKeyboard(entry));
      await tgClient.answerCallbackQuery(query.id, chosenLabels.length ? `已选 ${chosenLabels.join('、')}` : '已取消选择');
      return true;

    } catch (err) {
      log('warn', `Synthetic question callback failed: ${err.message}`);
      try { await tgClient.answerCallbackQuery(query.id, '操作失败'); } catch {}
      return true;
    }
  }
  ...
  async function finalizeStreamingReply(botId, chatId, rawText, opts = {}) {
    ...
    const detection = detectTextOptions(prefixed);
    if (detection) {
      log('info', `C-fallback: detected text options in streaming reply for chat ${chatId} (${detection.options.length} opts, ${detection.multiSelect ? 'multi' : 'single'})`);
      // Delete the placeholder (it was showing the raw text with options)
      if (placeholderMessageId) {
        try { await tgClient.deleteMessage(chatId, placeholderMessageId); } catch { /* ignore */ }
      }
      const cardId = await sendSyntheticQuestionCard(botId, chatId, detection, { messageThreadId });
      return cardId != null;
    }
  ...
  // v0.6.5: the bot an AGENT belongs to — the authoritative routing signal
  // when two bots share one chatId. Resolution per session (walking up the
  // parentSession chain for subagents): (a) a chatAgents entry routed to the
  // session id (the composite key carries the bot); (b) the session's own
  // meta.botId / session.header.botId recorded at creation time (covers
  // agents created but not currently routed, e.g. after a reload).
  // Returns null when nothing knows — callers then fall back to chatId
  // ownership (which is all the legacy single-bot path ever had).
  function botIdForAgent(agent) {
    const agentsSvc = ctx.get?.('agents');
    let sid = String(agent?.session?.id ?? agent?.id ?? '');
    if (!sid) return null;
    let cur = agent?.session;
    const seen = new Set();
    while (sid && seen.size < 64) {
      if (seen.has(sid)) return null;
      seen.add(sid);
      for (const [key, routed] of chatAgents) {
        if (String(routed) === sid) {
          const bi = key.indexOf('::');
          return bi >= 0 ? key.slice(0, bi) : 'default';
        }
      }
      const metaBot = agent?.meta?.botId ?? cur?.header?.botId;
      if (typeof metaBot === 'string' && metaBot) return metaBot;
...
With multiple bots configured, pass the "bot" parameter to choose which bot sends (default: the bot running this conversation, else the default/first bot).`,
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this message (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot when there is only one.',
      },
      text: {
        type: 'string',
        required: true,
    ...
    async execute(args, exec) {
      ...
      const { botId, error } = resolveBotClient(args.bot, chatId, exec?.agent);
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this photo (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      photo: {
        type: 'string',
        required: true,
    ...
    async execute(args, exec) {
      ...
      const { client: sendClient, error } = resolveBotClient(args.bot, chatId, exec?.agent);
      if (error) return error;
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this document (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      document: {
        type: 'string',
        required: true,
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this video (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      video: {
        type: 'string',
        required: true,
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this audio (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      audio: {
        type: 'string',
        required: true,
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot sends this voice message (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      text: {
        type: 'string',
        required: true,
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot performs the edit (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      message_id: {
        type: 'integer',
        required: true,
    ...
    async execute(args, exec) {
      const { client: sendClient, error } = resolveBotClient(args.bot, args.chat_id, exec?.agent);
      if (error) return error;
  ...
  registerTool({
    ...
    parameters: {
      ...
      bot: {
        type: 'string',
        description: 'Which bot performs the delete (a bot id). Default: the bot running this conversation (the one the user messaged), else the default/first bot.',
      },
      message_id: {
        type: 'integer',
        required: true,
  ...
  registerTool({
    ...
    // one entry (id "default"), so single-bot callers still see one item.
    // dsh 0.1.1-rc.1+ tool-output contract: native tool results must be strings
    // (dsh-tools validates the scalar), so serialize the entries array to
    // indented JSON instead of returning the raw array.
    async execute() {
  ...
  const sessionTitleSeeded = new Set(); // session ids already given a display title
  ...
  if (pollingEnabled && client) {
    ...
    async function createTelegramAgent(botId, chatId) {
      ...
      // v0.6.4: the host attaches a session to a workspace only inside its own
      // session.create route (apiproxy → workspace.attachSession). Agents
      // built here go through the agents service directly, so they were never
      // claimed by any workspace and the web sidebar dropped them into
      // "Ungrouped" (invisible under the Documents group) — the reported
      // "can't see telegram sessions in web" bug. Attach to the workspace
      // that owns `cwd`, best-effort.
      try {
        const wsSvc = ctx.workspaceRegistry ?? ctx.get?.('workspaceRegistry');
        const ws = wsSvc?.resolveByPath ? await wsSvc.resolveByPath(cwd) : undefined;
        if (ws?.attachSession) await ws.attachSession(sessionId);
      } catch (err) {
        log('warn', `createTelegramAgent: workspace attach failed for ${sessionId}: ${err.message}`);
      }
      return String(sessionId);
    }

    // v0.6.4: give a title-less session a display title from the user's first
    // Telegram text. The host title service only harvests `user/message`
    // events whose source.kind is 'user' (dsh-session-title
    // collectSessionTitleMessages); our injections are kind 'plugin' (correct
    // provenance — we must not fake them), so telegram sessions would forever
    // show a null title in the web sidebar. rename() appends a pinned
    // session/title event; since the service can never auto-title these
    // sessions, pinning costs nothing. Never overwrite an existing title.
    function seedSessionTitle(agent, text) {
      try {
        const titles = ctx.get?.('sessionTitle');
        const session = agent?.session;
        if (!titles?.rename || !session) return false;
        if (titles.get?.(session)?.title !== undefined) return true;
        const seed = String(text ?? '').replace(/\s+/g, ' ').trim().slice(0, 80);
        if (!seed) return false;
        titles.rename(session, seed);
        return true;
      } catch (err) {
        log('warn', `seedSessionTitle failed: ${err.message}`);
        return false;
      }
    }
    ...
    function resolveChatAgent(botId, chatId) {
      ...
      if (activeId) {
        ...
        if (hit) return hit;
        // The active agent disappeared (disposed). Do NOT fall back to all[0]:
        // under multi-bot both bots share the same chat, so all[0] is typically
        // the OTHER bot's agent — that cross-bot fallback is exactly the
        // "command sent to bot B landed in bot A's session" bug. Return null;
        // the inbound path (handleIncomingMessage) auto-creates a fresh agent
        // for this (bot, chat) instead.
        return null;
      }
      // No agent routed for this (bot, chat). Returning all[0] would let the
      // second bot in a shared chat inherit the first bot's session. Return
      // null so the caller decides (inbound messages auto-create; commands
      // report "no session yet").
      return null;
    }

    // Create the (bot, chat)'s own agent if none is routed yet, and route the
    // chat to it. Async because agentsSvc.create awaits. Returns the agent
    // handle, or null when the agents service is unavailable. The inbound
    // message path calls this so a second bot in a shared chat never inherits
    // the first bot's session (multi-bot cross-routing bug).
    async function ensureChatAgent(botId, chatId) {
      const existing = resolveChatAgent(botId, chatId);
      if (existing) return existing;
      const agentsSvc = ctx.get?.('agents');
      if (!agentsSvc?.create) return null;
      try {
        const id = await createTelegramAgent(botId, chatId);
        chatAgents.set(k(botId, chatId), id);
        const hit = (agentsSvc.list?.() ?? []).find((a) => String(a?.session?.id) === id);
        return hit ?? null;
      } catch (err) {
        log('warn', `ensureChatAgent: create failed for (bot=${botId}, chat=${chatId}): ${err.message}`);
        return null;
      }
    }
    ...
    // The agent a chat routes to (its active one, else null). Strict: NO
    // all[0] fallback — under multi-bot a shared chat would otherwise show
    // the OTHER bot's agent as this (bot, chat)'s board parent.
    ...
    function boardForChat(botId, chatId) {
      ...
      if (!b) {
        ...
        b = new SubagentBoard({
          ...
          deleteMessage: (messageId) => bClient.deleteMessage(cid, messageId),
    ...
    // v0.5.x P2: autopilotChats is keyed by k(botId, chatId).
    // v0.6.2: the module-facing helpers take (chatId, botId?) — the
    // approval/question modules now receive the owning bot from ownership().
    // With botId present we match the EXACT (bot, chat) key, so two bots sharing
    // one chat no longer leak autopilot state into each other. Without botId we
    // keep the legacy scan (any bot) so bare-chatId callers still work.
    function autopilotStateFor(chatId, botId) {
      const cid = String(chatId);
      if (botId) {
        const state = autopilotChats.get(k(botId, cid));
    ...
    function isAutopilotChat(chatId, botId) {
      return !!autopilotStateFor(chatId, botId);
    }
    ...
    function enableAutopilot(botId, chatId, agent) {
      ...
      // (later auto-grants stay silent; see approval.js). Pass botId so only
      // THIS bot's notice flag is cleared (shared chat, multi-bot).
      approvalClearNotice?.(chatId, botId);
      return state;
    }
    ...
    function disableAutopilot(botId, chatId, agent) {
      ...
      approvalClearNotice?.(chatId, botId); // cleanup: no notice state lingers (this bot only)
      if (state?.on && agent) {
    ...
    // Returns { chatId, botId, threadId } when the question's session belongs to
    // an autopilot chat we own, else null (the normal wait-for-user flow runs).
    // v0.6.2: botId is the (bot, chat) key part — the chat is "in autopilot" only
    // for the bot that turned it on, not for another bot sharing the same chat.
    function autopilotOwnership(sessionId) {
      ...
      for (const [key, csid] of chatAgents) {
        if (String(csid) === hitSid) {
          ...
          const botPart = bi >= 0 ? key.slice(0, bi) : null;
          if (isAutopilotChat(chatPart, botPart)) {
            return { chatId: chatPart, botId: botPart, threadId: null };
          }
        }
      }
      if (defaultChatId && isAutopilotChat(defaultChatId, botIdForChat(String(defaultChatId)) || firstBotId)) {
        const defId = String(all[0]?.session?.id ?? '');
        if (defId && hitSid === defId) {
          return { chatId: String(defaultChatId), botId: botIdForChat(String(defaultChatId)) || firstBotId || 'default', threadId: null };
        }
      }
      return null;
    }
    ...
    async function handleCommands(botId, message) {
      ...
      // `s-xxxx` handles for every live session, collision-resolved once so
      // every command renders and matches ids the same way.
      const shortIds = sessionShortIds(all.map((a) => String(a?.session?.id ?? '')));
      const fmtId = (id) => shortIds.get(String(id ?? ''))?.short
        ?? sessionShortIds([String(id ?? '')]).get(String(id ?? ''))?.short
      ...
      try {
        switch (cmd) {
          ...
          case '/clear': {
            ...
            const id = await createTelegramAgent(botId, chatId);
            chatAgents.set(k(botId, chatId), id);
            await reply(`✅ 已开启新会话 <code>${escapeHtml(fmtId(id))}</code>（完整 id 见 /sessions）。\n上下文已清空，后续消息路由到新会话。`);
            return true;
          }
          ...
          case '/sessions': {
            ...
            const lines = all.map((a, i) => {
              ...
              const sShort = shortIds.get(aid)?.short ?? 's-????';
              const status = escapeHtml(a?.status ?? '?');
              let title = '';
              try { title = String(titlesSvc?.get?.(a?.session)?.title ?? '').trim(); } catch { /* best-effort */ }
              const tag = i === 0 ? '<i> (default)</i>' : '';
              const head = `${mark} <code>${escapeHtml(sShort)}</code> [${status}]${tag}`;
              if (!title) return `${head} <i>(未命名)</i>`;
              const wrapped = wrapDisplay(title, 42, 2).map((l) => escapeHtml(l));
          ...
          case '/use': {
            const targetArg = String(args ?? '').trim();
            if (!targetArg) { await reply('用法：/use &lt;s-id&gt;（先 /sessions 查看）'); return true; }
            // Accept: full id, `s-xxxx` short handles from /sessions, or a
            // raw prefix of the id / its hex core.
            const needle = targetArg.replace(/^s-/i, '').toLowerCase();
            const hit = all.find((a) => {
              const aid = String(a?.session?.id ?? '');
              if (aid === targetArg) return true;
              const core = shortIds.get(aid)?.core ?? '';
              return (needle.length >= 4 && core.startsWith(needle)) || aid.toLowerCase().startsWith(targetArg.toLowerCase());
            });
            const hitShort = hit ? (shortIds.get(String(hit.session.id))?.short ?? String(hit.session.id).slice(0, 12)) : '';
            if (!hit) { await reply(`❌ 未找到会话 <code>${escapeHtml(targetArg)}</code>。用 /sessions 查看。`); return true; }
            chatAgents.set(k(botId, chatId), String(hit.session.id));
            await reply(`✅ 已切换到会话 <code>${escapeHtml(hitShort)}</code>。`);
            return true;
          }
          ...
          case '/history': {
            ...
            await reply(`📜 <b>最近对话</b>（${escapeHtml(fmtId(active.session.id))}）\n\n${formatSessionHistory(active, n)}`);
            return true;
          }
          ...
          case '/model': {
            ...
            await reply(`✅ 会话 <code>${escapeHtml(fmtId(target.session.id))}</code> 的模型已切换为 <code>${escapeHtml(provider)}:${escapeHtml(model)}</code>（下一轮请求生效）。`);
            return true;
          }
          ...
          case '/autopilot': {
            ...
            if (wantOff) {
              ...
              await reply(`🛡️ 已关闭 autopilot（会话 <code>${escapeHtml(fmtId(agent?.session?.id))}</code>）。\n权限已恢复为 <code>${escapeHtml(s.prevSandbox ?? 'workspace-write')}</code> + 需审批，提问会重新等待你选择。`);
              return true;
            }
            ...
            const short = fmtId(agent?.session?.id);
    ...
    // Inbound-media module (extracted to inbound-media.js): detect kind, pick
    // file_id, download, and build a note (+ optional vision block). deps are
    // the per-apply() bindings: clientFor resolves the source bot's client,
    // log/inboundMediaDir/inboundImageToModel are the config-derived values.
    const inboundMedia = createInboundMediaModule({
      clientFor,
      log,
      inboundMediaDir,
      inboundImageToModel,
      ctx, // used only to resolve the `attachments` service for vision blocks
    });
    ...
    async function handleIncomingMessage(botId, message) {
      ...
      // v0.6.2: pass botId so the plain text only matches a card for THIS bot —
      // a reply to bot B is no longer consumed as bot A's pending answer in a
      // shared chat.
      if (message.text && questionConsumeText && questionConsumeText(message.chatId, botId, message.text)) {
      ...
      if (message.text || hasMedia) {
        ...
        // Inject message to agent session if enabled. If no agent is routed
        // for this (bot, chat) yet, create one NOW instead of falling back to
        // another bot's session (multi-bot shared-chat cross-routing bug).
        let currentAgent = resolveChatAgent(botId, message.chatId);
        if (injectToAgent && !currentAgent) {
          currentAgent = await ensureChatAgent(botId, message.chatId);
        }
        // Inbound routing trace (verbose only) — the deaf-bot investigation
        // showed agent-shape drift is the first thing to check.
        log('debug', 'inbound routed', JSON.stringify({
          botId, chatId: String(message.chatId), injectToAgent,
          agent: currentAgent ? {
            id: String(currentAgent.session?.id), status: currentAgent.status,
            followup: typeof currentAgent.followup, send: typeof currentAgent.send,
            inboxAppend: typeof currentAgent.inbox?.append, wakeDriver: typeof currentAgent.wakeDriver,
          } : null,
        }));
        ...
        if (injectToAgent && currentAgent) {
          try {
            ...
            if (hasMedia && forwardInboundMedia) {
              const fm = await inboundMedia.downloadAndDescribeInboundMedia(botId, message);
              mediaNote = fm.note;
              ...
              if (inboundMedia.inboundMediaKind(message) === 'voice' && voiceTranscribe && fm.localPath) {
            ...
            const autopilotHint = isAutopilotChat(message.chatId, botId)
            ...
            if (hasMedia) {
              bodyParts.push(forwardInboundMedia ? mediaNote : `The user sent a ${inboundMedia.inboundMediaKind(message)}.`);
            }
            ...
            if (typeof currentAgent.followup === 'function') {
              ...
              log('debug', 'followup() called for', String(currentAgent.session?.id));
            } else if (typeof currentAgent.send === 'function') {
              currentAgent.send(userMessage, 'next-turn', true);
              log('debug', 'send() called for', String(currentAgent.session?.id));
            } else {
              currentAgent.inbox.append('next-turn', userMessage);
              currentAgent.wakeDriver?.(true);
            }
            rememberInjectedId(dedupKey);
            if (String(currentAgent.session?.id ?? '').startsWith(TELEGRAM_SESSION_PREFIX)
                && !sessionTitleSeeded.has(String(currentAgent.session.id))
                && seedSessionTitle(currentAgent, message.text ?? '')) {
              sessionTitleSeeded.add(String(currentAgent.session.id));
            }
        ...
        const echoText = message.text
          ...
          : `I received a ${inboundMedia.inboundMediaKind(message)}.`;
        ...
        if (sidStr.startsWith(TELEGRAM_SESSION_PREFIX)) {
          ...
          for (const [key, csid] of chatAgents) {
            if (String(csid) === sidStr) {
              ...
              botId = bi >= 0 ? key.slice(0, bi) : null;
              break;
            }
          }
          ...
          if (!chatId && hit?.meta?.botId) {
            ...
            if (!botId) botId = hit.meta.botId;
          }
          if (!chatId && defaultChatId) chatId = String(defaultChatId);
          if (!chatId) return null;
          // v0.6.2: carry the owning bot so callers (autopilot / consume /
          // allow-always) key per (bot, chat) — not bare chatId — and two bots
          // sharing one chat never leak state into each other.
          return { chatId, botId, threadId: null };
        }

        // Case 2 — a NON-telegram agent (typically the deployment's DEFAULT
        // shared web agent) that some (bot, chat) explicitly routes to via
        // /use. Claim it only when a chatAgents entry actually points at it:
        // since v0.6.2 resolveChatAgent never falls back to all[0] (inbound
        // messages auto-create telegram-* agents), so the old "pre-/new
        // messages land on the shared agent" premise is gone and claiming
        // all[0] unconditionally pushed the shared WEB agent's questions to
        // the phone — the reported bug. Gated by the same opt-in flags
        // (approvalForDefaultAgent / questionsForDefaultAgent).
        if (allowDefault && !sidStr.startsWith(TELEGRAM_SESSION_PREFIX)) {
          for (const [key, csid] of chatAgents) {
            if (String(csid) !== sidStr) continue;
            const bi = key.indexOf('::');
            const chatId = bi >= 0 ? key.slice(bi + 2) : key;
            const botId = bi >= 0 ? key.slice(0, bi) : null;
            return { chatId, botId: botId || botIdForChat(chatId) || firstBotId || 'default', threadId: null };
          }
        }
    ...
    if (approvalEnabled && typeof ctx.on === 'function') {
      const approvalModule = createApprovalModule({
        ...
        // Autopilot: auto-grant every ask for a (bot, chat) in autopilot mode
        // (no card). v0.6.2: keyed by the owning bot so a shared chat does not
        // leak one bot's autopilot into the other.
        ...
        checkAllow: (ruleKey, chatId) => approvalStore?.checkAllow(ruleKey, chatId) ?? false,
      ...
      approvalClearNotice = (chatId, botId) => { try { approvalModule.clearAutopilotNotice(chatId, botId); } catch { /* ignore */ } };
    ...
    if (questionsEnabled && activeClient && typeof ctx.on === 'function') {
      const questionModule = createQuestionModule({
        ...
        isAutopilot: (chatId, botId) => isAutopilotChat(chatId, botId),
        // The module knows the (bot, chat) from ownership(); takeover disables
        // autopilot for EXACTLY that (bot, chat) — no cross-bot fallback.
        autopilotTakeover: (chatId, botId) => {
          const bid = botId || botIdForChat(chatId);
    ...
    const handleCallbackQuery = async (botId, query) => {
      ...
      // v0.6.2: the wrapper's `botId` IS the source bot (each per-bot poller
      // passes its own id; the legacy single-poller path passes firstBotId).
      // Prefer it over botIdForChat(chatId), which in a shared chat returns
      // whichever bot registered first — that misroutes answerCallbackQuery
      // (the ack toast) to the WRONG bot's client.
      if (qChat != null) setActiveCardBotId(botId || botIdForChat(qChat));
      ...
      // Tool-guard approval first, then ask_user_question, then synthetic C-fallback.
      // Anything else falls through to a plain ack on the SOURCE bot.
      if (approvalHandleQuery && approvalHandleQuery(query)) return;
      if (questionHandleQuery && (await questionHandleQuery(query))) return;
      if (await handleSyntheticQuestionCallback(botId, query)) return;
  ...
  // v0.6.4 one-time backfill: telegram agents created before this fix were
  // never claimed by any workspace (the host only attaches inside its own
  // session.create route), so they sat in "Ungrouped" — invisible under the
  // sidebar's workspace groups. Walk persisted `telegram-*` session headers
  // and attach each to the workspace that owns its cwd. attachSession is a
  // no-op for already-attached sessions, so this sweep is idempotent across
  // restarts. Fire-and-forget: never delays plugin startup.
  void (async () => {
    try {
      const svc = (n) => { try { return ctx.get?.(n); } catch { return undefined; } };
      let sp = svc('sessionPersistence');
      let wsSvc = svc('workspaceRegistry');
      for (let i = 0; i < 60 && !(sp?.list && wsSvc?.resolveByPath); i++) {
        await new Promise((r) => setTimeout(r, 1000));
        sp = svc('sessionPersistence');
        wsSvc = svc('workspaceRegistry');
      }
      if (!sp?.list || !wsSvc?.resolveByPath) return;
      const headers = await sp.list();
      // A "blank" telegram session (created by /new or auto-start but never
      // receiving a prompt) must NOT be attached: the web UI treats any blank
      // session inside a workspace as its provisional "New Session" draft and
      // routes new prompts into it (ui-workspace connectWorkspace reuse),
      // which leaks desktop-side chats into stale telegram-* logs. Blank =
      // persisted log carries no user/message event.
      const isBlank = async (id, header) => {
        try {
          const loc = sp.locate?.(header);
          const p = loc?.path;
          if (!p) return false;
          const st = statSync(p);
          if (st.size >= 65536) return false; // host's own cold-blank probe bound
          const out = spawnSync('zstd', ['-q', '-dc', p], { encoding: 'utf8', timeout: 15000 });
          const text = out.stdout || '';
          return !text.includes('"type":"user/message"');
        } catch { return false; }
      };
      const byPath = new Map();
      for (const h of headers ?? []) {
        if (!String(h?.id ?? '').startsWith(TELEGRAM_SESSION_PREFIX) || !h?.cwd) continue;
        if (await isBlank(String(h.id), h)) continue;
        (byPath.get(h.cwd) ?? byPath.set(h.cwd, []).get(h.cwd)).push(String(h.id));
      }
      let attached = 0;
      for (const [path, ids] of byPath) {
        let ws;
        try { ws = await wsSvc.resolveByPath(path); } catch { continue; }
        if (!ws?.attachSession) continue;
        for (const id of ids) {
          try { await ws.attachSession(id); attached++; } catch { /* header gone / cwd moved */ }
        }
      }
      if (attached) log('info', `Backfilled ${attached} telegram session(s) into their workspaces.`);

      // One-time sweep: archive blank telegram sessions that earlier backfill
      // versions attached (the "新会话串号" bug). Idempotent via marker file;
      // never touches live sessions; session files themselves are kept.
      const marker = join(homedir(), '.dsh', '.telegram-blank-archive.done');
      try {
        const arch = wsSvc?.archiveSession && !existsSync(marker);
        if (arch) {
          let archived = 0;
          for (const h of headers ?? []) {
            const id = String(h?.id ?? '');
            if (!id.startsWith(TELEGRAM_SESSION_PREFIX)) continue;
            let live = false;
            try { live = !!svc('sessions')?.get?.(id); } catch { /* not injected */ }
            if (live) continue; // live — leave alone
            if (!(await isBlank(id, h))) continue;
            try { await wsSvc.archiveSession(id); archived++; } catch { /* unknown session */ }
          }
          writeFileSync(marker, new Date().toISOString() + '\n', 'utf8');
          if (archived) log('info', `Archived ${archived} blank telegram session(s) from the sidebar (blank-new-session leak cleanup).`);
        }
      } catch (err) {
        log('warn', `blank-session archive sweep failed: ${err.message}`);
      }
    } catch (err) {
      log('warn', `workspace backfill failed: ${err.message}`);
    }
  })();
