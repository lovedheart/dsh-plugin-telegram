...
export function buildQuestionCard(questions, escape, selections = new Map()) {
  ...
  questions.forEach((q, qi) => {
    ...
    const labeled = opts.map((o, idx) => ({
      idx,
      text: String(o?.label || '').slice(0, 40) || `选项 ${idx + 1}`,
      desc: o?.description ? String(o.description) : '',
    }));
    // Option details in the message body: models frequently put the real
    // substance in option.description, which used to be silently dropped, so
    // the card looked "incomplete". Numbering follows button order.
    const withDesc = labeled.filter((o) => o.desc);
    if (withDesc.length) {
      parts.push('📝 选项说明（按按钮顺序）：');
      for (const o of withDesc) parts.push(`${o.idx + 1}. ${esc(o.text)}：${esc(o.desc)}`);
    }
    // Adaptive row packing: long CJK labels get one button per row (never
    // clipped on a phone); short labels (是/否/OK) still share a row.
    const perRow = labeled.length
      ? buttonsPerRow(Math.max(...labeled.map((o) => displayWidth(o.text))))
      : 1;
    for (let i = 0; i < labeled.length; i += perRow) {
      keyboard.push(labeled.slice(i, i + perRow).map((o) => ({
        text: o.text,
...
export function createQuestionModule(deps) {
  ...
  // Re-render the card's text AND keyboard after a selection changes. Re-sending
  // reply_markup forces Telegram clients to re-render the keyboard — a text-only
  // editMessageText can leave stale/missing keyboard rows on some clients, which
  // hid the trailing [✅ 提交][❌ 取消] row and left users unable to submit.
  function refreshCard(entry) {
    ...
    deps.client
      .editMessageText(entry.chatId, entry.cardMessageId, text, 'HTML', { inline_keyboard: real })
      .catch(() => {});
  }
  ...
  async function handleCallbackQuery(query) {
    ...
    if (q.multiSelect) {
      ...
      await ack('🔘');
      refreshCard(entry); // text + keyboard re-render
    } else {
