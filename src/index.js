    ...
    const telegramAgentOwnership = (sessionIdLike, { allowDefault = true } = {}) => {
      ...
      const sidStr = String(hit?.session?.id ?? askId ?? '');
        ...
        if (allowDefault && defaultChatId && hit === all[0]) {
          // v0.6.2: the shared default agent is bot-agnostic; its per-chat state
          // (autopilot / allow-always / text-consume) is keyed by the RESOLVING
          // bot (the one that routed the default agent for this chat). When no
          // bot has routed it yet (first message pre-creation), fall back to the
          // first bot. Legacy single-bot config → 'default' (behavior unchanged).
          return { chatId: String(defaultChatId), botId: botIdForChat(String(defaultChatId)) || firstBotId || 'default', threadId: null };
        }
