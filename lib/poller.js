...
export class TelegramPoller {
  ...
  async _processUpdates(updates) {
    // IMPORTANT: dispatch each update's handlers WITHOUT awaiting them inside
    // the poll loop. A handler that stalls on a hung/flaky Telegram call (see
    // the per-request timeout in client.js) must never be allowed to block the
    // next getUpdates — that was the "bot deaf during a long task" bug. The
    // handlers run concurrently in the background; per-handler errors are
    // logged, not thrown, so one bad update can't taint the batch.
    for (const update of updates) {
      try {
        ...
        if (msg && this.messageHandlers.length > 0) {
          ...
          // Fire-and-forget: do NOT await. A hung handler cannot stall the
          // poll loop. Wrap each handler so a sync throw can't break the map.
          const settled = this.messageHandlers.map((h) =>
            Promise.resolve().then(() => h(msg)).catch((err) =>
              this._log('error', 'Message handler error:', err),
            ),
          );
        ...
        if (update.callbackQuery && this.callbackHandlers.length > 0) {
          const settled = this.callbackHandlers.map((h) =>
            Promise.resolve().then(() => h(update.callbackQuery)).catch((err) =>
              this._log('error', 'Callback handler error:', err),
            ),
          );
          void Promise.allSettled(settled);
        }
