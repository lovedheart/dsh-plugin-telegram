...
export function latestActivity(session, sinceSeq = 0) {
  ...
  let reasoning = '';
  let text = '';
  // Recency, not type priority: an old tool call must not outrank NEWER
  // reasoning/text — track each candidate's event seq and return the most
  // recent one (this is the board's "what is it doing right now").
  let seqTool = -1;
  let seqReasoning = -1;
  let seqText = -1;
  for (const evt of events) {
    ...
    if (type === 'tool/call') {
      ...
      if (chunk.type === 'reasoning-delta' && typeof chunk.text === 'string') {
        // Reset at each NEW reasoning block so separate blocks don't concatenate.
        if (evt.seq !== seqReasoning + 1) reasoning = '';
        reasoning += chunk.text;
        seqReasoning = evt.seq;
      ...
      if (msg && Array.isArray(msg.content)) {
        ...
        if (t) { text = t; seqText = evt.seq; }
      }
    }
  }
  const picks = [[seqTool, tool], [seqReasoning, reasoning.trim()], [seqText, text]]
    .filter(([, v]) => v)
    .sort((a, b) => b[0] - a[0]);
  return picks[0]?.[1] || '';
}
...
export class SubagentBoard {
  constructor(deps) {
    ...
    // Optional: board message deletion on teardown.
    this.deleteMessage = typeof deps.deleteMessage === 'function' ? deps.deleteMessage : null;
  ...
  onStart(info) {
    ...
    e.stopReason = '';
    e.endedAt = 0;
    // Re-start = new epoch: an inherited startedAt would sort the row wrong
    // and a stale missedTicks could lock it completed on the very next tick.
    e.startedAt = this.clock();
    e.missedTicks = 0;
    e.nonRunningTicks = 0;
  ...
  refresh(parentSession) {
    ...
    try {
      ...
      // A transient read failure is NOT "no agents": advancing missedTicks
      // here would falsely complete every working child after GRACE_TICKS.
      this.log('warn', 'subagent board: agents list unavailable this tick; keeping state');
      return;
    }
    ...
    let workingCount = 0;
    for (const e of this.entries.values()) if (e.status === 'working') workingCount += 1;
    ...
    for (const e of this.entries.values()) {
      ...
      if (child) {
        ...
        if (child.status && child.status !== 'running') {
          const st = String(child.status).toLowerCase();
          const terminal = ['completed', 'error', 'failed', 'aborted', 'cancelled', 'canceled'].includes(st);
          // idle/inactive is NOT terminal (a continuable child between turns);
          // lock only after the state persists for the usual grace window.
          e.nonRunningTicks = (e.nonRunningTicks || 0) + 1;
          if (terminal || e.nonRunningTicks >= GRACE_TICKS) {
            if (e.status === 'working') {
              e.status = terminal ? normalizeStopReason(st) : 'completed';
            }
            e.locked = true;
        ...
        // parentDesc.last is a GLOBAL fallback (the parent's most recent
        // subagent tool call) — with several children it would label rows
        // with another child's description. Only use it while exactly one
        // child is working; otherwise fall back to the child's own id.
        const soloWorking = parentDesc.last && workingCount === 1;
        const label = labelFromSession(session) || parentDesc.byId.get(e.id)
          || (soloWorking ? parentDesc.last : '') || e.label || e.id;
        e.label = label;
  ...
  async flush(force = false) {
    ...
    try {
      if (!(Number.isFinite(this.messageId) && this.messageId > 0)) {
        ...
        if ((this._gen || 0) !== gen) return; // teardown during the await
        if (!ok) {
      ...
      if ((this._gen || 0) !== gen) return; // teardown during the awaits
      this.lastRendered = text;
      this.lastFlushAt = this.clock();
      this.dirty = false;
    } catch (err) {
      // `err` may be a thrown non-object (null/undefined/string).
      this.log('warn', `subagent board flush failed: ${err?.message ?? String(err)}`);
      this.dirty = true;
    } finally {
      this.flushInFlight = false;
    }
  ...
  async teardown() {
    // Generation bump invalidates any in-flight flush so it cannot repopulate
    // messageId (or an orphan board message) for the retired session.
    ...
    try { await this.deleteMessage?.(id); } catch { /* best-effort */ }
  }
}
