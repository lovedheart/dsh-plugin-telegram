...
With multiple bots configured, pass the "bot" parameter to choose which bot sends (default: the bot that owns the chat, or the default/first bot).`,
  ...
  if (pollingEnabled && client) {
    ...
    async function handleCommands(botId, message) {
      ...
      try {
        switch (cmd) {
          ...
          case '/call': {
            // 📞 voice-call Mini App shortcut. Tapping the link opens the
            // Mini App in the Telegram in-app browser (same URL as the chat
            // menu button). URL configurable via `voiceCallUrl`.
            const url = c.voiceCallUrl || 'https://voice.ez8.dev/app';
            await reply(`📞 点下方链接开始语音通话：\n${url}`);
            return true;
          }
    ...
    const commandMenu = [
      ...
      { command: 'call', description: '📞 开始语音通话（Mini App）' },
