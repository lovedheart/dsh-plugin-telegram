...
/**
 * Telegram may answer HTTP 200 with `{"ok":false,...}` — treat it as the
 * error it is (with the real error_code/description), instead of letting
 * callers see a result-less success.
 */
function assertTelegramOk(method, data) {
  if (data && data.ok === false) {
    const code = typeof data.error_code === 'number' ? data.error_code : undefined;
    const description = data.description || `${method} failed (ok:false)`;
    if (code === 429) {
...
async function tgFetchOk(baseUrl, botToken, method, body, signal, timeoutMs) {
  ...
  return assertTelegramOk(method, data);
}
...
async function tgUploadFile(baseUrl, botToken, method, field, chatId, filePath, extra = {}, signal, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
  // Async read: a full-file readFileSync on the send path blocks the event
  // loop for the whole (up to ~50 MB) upload — the poll loop goes deaf.
  ...
  assertTelegramOk(`${method} (upload)`, data2);
...
export class TelegramClient {
  constructor(config) {
    ...
    // Optional upload confinement roots (see _localPathOr). Unset = open.
    this.uploadRoots = Array.isArray(config.uploadRoots) && config.uploadRoots.length
      ? config.uploadRoots.map((r) => String(r))
      : null;
  ...
  async editMessageText(chatId, messageId, text, parseMode, replyMarkup, opts) {
    try {
      ...
      // A network/timeout/5xx failure is RETRYABLE — reporting it as `false`
      // (permanent) made callers drop the edit silently. Surface it.
   ...
   *
   * When `config.uploadRoots` is set, the REAL path (symlinks resolved) must
   * stay inside one of those roots — confinement survives symlink traversal,
   * so a prompt-injected tool call cannot exfiltrate e.g. ~/.ssh keys by
   * pointing `photo` at an escaping link. Unset (default) keeps the
  ...
  _localPathOr(ref) {
    ...
    const roots = this.uploadRoots;
    if (roots && roots.length) {
      let real = abs;
      try { real = realpathSync(abs); } catch { /* keep abs */ }
      const inside = roots.some((r) => {
        let root = r;
        try { root = realpathSync(r); } catch { root = resolve(r); }
        return real === root || real.startsWith(root + sep);
      });
      if (!inside) {
        throw new Error(`upload path ${real} is outside the configured upload roots`);
      }
      return real;
    }
    return abs;
  ...
  async getUpdates(offset, limit, timeout, signal) {
    // Clamp to Telegram's documented ranges; an out-of-range value is a hard
    // 400 that would otherwise look like a silent empty batch.
    limit = Math.min(100, Math.max(1, Number(limit) > 0 ? Number(limit) : 50));
    timeout = Math.min(60, Math.max(0, Number.isFinite(Number(timeout)) ? Number(timeout) : DEFAULT_LONG_POLL_TIMEOUT));
  ...
  _parseMessage(raw) {
    ...
    return {
      ...
      // Channel posts / service messages carry no `from` — keep it undefined
      // instead of the literal string "undefined" (which poisoned sender
      // based allowlists, dedup and routing).
