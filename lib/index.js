...
const defaults = {
  ...
  // Progress indicator (tool calls + thinking) — in BOTH 'direct' and 'tool'
  // response modes. While the agent works on a Telegram message, keep a single
  // editable message alive that shows what it is doing right now (a tool call
  // in flight or active thinking) plus a live "typing…" chat action, so the
  // user is never left staring at silence. Modelled on QwenPaw's Telegram
  // channel streaming + typing hooks. The message is deleted as soon as the
  // turn ends (turn/end); progressTimeoutSec is an absolute self-cleaning cap.
  // Short turns that finish before progressDelaySec never show the indicator.
  ...
  progressTailChars: 240,    // max chars of the thinking/reply tail to show
...
/**
 * Live Telegram progress indicator for one agent turn.
 *
 * Options `o`: { chatId, threadId, agent, baseline, startedAt, client, log,
 * delayMs, intervalMs, tailChars, timeoutMs }. `agent.session.events` is the
 * durable, growing session log; only events with `seq > baseline` belong to
 * this turn. `client` is a TelegramClient (or a test double exposing
 * sendMessage/editMessageText/deleteMessage/sendChatAction). `log` is a
 * `(level, ...args)` logger.
 */
export class ProgressIndicator {
  constructor(o) {
    ...
    // Which activity happened most recently: 'reasoning' | 'tool' | 'text' |
    // null. The indicator reflects the LATEST activity, not a fixed priority —
    // so once the model stops thinking and starts the reply, we show the reply
    // (not stale thinking).
    this.lastKind = null;
    this.loop = null;
    // Watermark: only events with seq strictly greater than this are new to
    // this indicator. We fold the whole log on every tick but skip already
    // seen events, so state is never double-applied.
    this.processedSeq = this.o.baseline ?? 0;
  }

  /** The current activity line, e.g. "🔧 正在调用工具 bash：ls -la …". */
  buildStatusLine() {
    const { tailChars } = this.o;
    if (this.lastKind === 'tool' && this.currentTool) {
      const arg = summarizeToolArgs(this.currentTool.args);
      return `🔧 正在调用工具 ${this.currentTool.name}${arg ? `：${arg}` : ''}`;
    }
    if (this.lastKind === 'text' && this.replyBuf.trim()) {
      return `✍️ 正在写回复：${tailOf(this.replyBuf, tailChars)}`;
    }
    if (this.lastKind === 'reasoning' && this.reasoningBuf.trim()) {
      return `💭 思考中：${tailOf(this.reasoningBuf, tailChars)}`;
    }
    return '⏳ 正在处理，请稍候…';
  }
  ...
  /**
   * Fold new session events into the indicator state, in chronological order
   * (so the most recent activity wins). Only events with seq > processedSeq
   * are applied, so the full log can be passed on every tick without
   * re-processing. Returns true when the turn has ended (a turn/end was seen).
   */
  processEvents(events) {
    for (const evt of events) {
      ...
      if (type === 'reasoning-chunks') {
        ...
        if (Array.isArray(d.texts)) this.reasoningBuf += d.texts.join('');
        if (this.reasoningBuf.length > 4000) this.reasoningBuf = this.reasoningBuf.slice(-2000);
      } else if (type === 'text-chunks') {
        this.lastKind = 'text';
        if (Array.isArray(d.texts)) this.replyBuf += d.texts.join('');
        if (this.replyBuf.length > 4000) this.replyBuf = this.replyBuf.slice(-2000);
      } else if (type === 'tool-call-chunks') {
        // Tool arguments are streaming in; show the tool name as soon as known.
        this.lastKind = 'tool';
        if (d.name) this.currentTool = { name: d.name, args: this.currentTool?.args ?? '' };
      } else if (type === 'tool/call') {
        this.lastKind = 'tool';
        this.currentTool = { name: d.name ?? 'tool', args: d.arguments };
      } else if (type === 'tool/result') {
        // The tool finished; until the model's next block (thinking or reply)
        // starts, we are just "waiting" — show the neutral line rather than a
        // stale "calling tool X".
        this.lastKind = null;
...
export async function apply(ctx, config) {
  ...
  const progressTailChars = Math.max(40, Number(c.progressTailChars) || 240);
