 ...
 * @param {(agent) => ({chatId: string, botId?: (string|null), threadId: (number|null), chatLabel: string})|null}
 *        deps.ownership  resolve the owning chat for an agent; null = not ours
 * @param {(chatId: string, botId?: (string|null)) => boolean} [deps.isAutopilot]
...
export function createApprovalModule(deps) {
  ...
  function postAutopilotNotice(ownership, label, deps) {
    ...
    // v0.6.2: key the "already notified" flag by (bot, chat) so two bots sharing
    // one chat each get their own one-shot notice (the ownership carries the
    ...
    Promise.resolve()
      .then(() => deps.client.sendMessage({
        ...
        botId: ownership.botId ?? undefined, // v0.6.2: route to the owning bot (shared chat)
        text,
        parseMode: 'HTML',
  ...
  async function handleApprovalRequest(req, next) {
    ...
    if (deps.isAutopilot?.(ownership.chatId, ownership.botId)) {
    ...
    const entry = {
      ...
      ruleKey,
      chatId: ownership.chatId,
      botId: ownership.botId ?? null, // v0.6.2: owning bot for (bot, chat)-scoped routing
  ...
  /**
   * Forget the "already notified" flag for a (bot, chat) so the next auto-allowed
   * ask posts the notice again. Called by enableAutopilot (fresh /autopilot cycle)
   * and disableAutopilot (cleanup). v0.6.2: keyed by (bot, chat) when botId is
   * given, else bare chatId (legacy) — matching postAutopilotNotice.
   */
  function clearAutopilotNotice(chatId, botId) {
    const cid = String(chatId ?? '');
    autopilotNotified.delete(botId != null ? `${botId}::${cid}` : cid);
  }
